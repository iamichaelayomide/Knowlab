...
export default function PatientsPage() {
  ...
  // CRITICAL FIX: Ensure we don't return null and break the entire layout
  ...
  return (
    <div className="kl-page min-h-full max-w-full overflow-x-hidden p-6 sm:p-8 lg:p-10">
      ...
      <div className="mb-10 flex min-w-0 flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            onClick={() => openFloatingAI(`Summarize patient laboratory priorities and detailed history.`)}
          >
      ...
      <div className="grid min-w-0 gap-8 xl:grid-cols-[minmax(320px,380px)_minmax(0,1fr)]">
        {/* Sidebar */}
        <aside className="kl-premium-card min-w-0 p-5 h-[calc(100vh-200px)] overflow-y-auto shadow-md border-[var(--surface-border)]">
          ...
          <div className="space-y-4">
            {filteredPatients.length === 0 ? (
                <div className="py-12 text-center opacity-40 flex flex-col items-center">
                    <SearchNormal1 size={32} className="mb-2" />
        ...
        <main className="min-w-0">
          {!selectedPatient ? (
             <div className="h-full flex items-center justify-center flex-col text-[var(--text-tertiary)] py-20 bg-[var(--surface-card)] rounded-[40px] border border-[var(--surface-border)] border-dashed opacity-50">
                <Profile2User size={64} className="mb-4" />
                <p className="text-[18px] font-black uppercase tracking-[0.2em]">No patient selected</p>
                <p className="text-[14px] mt-2 font-medium">Select a patient from the sidebar to view clinical records.</p>
             </div>
          ) : (
          <div className="space-y-8">
            <section className="kl-gradient-card kl-premium-card min-w-0 p-8 sm:p-10 shadow-2xl rounded-[40px]">
                ...
                <div className="grid min-w-0 gap-8 lg:grid-cols-[minmax(340px,400px)_minmax(0,1fr)]">
                    ...
                    <div className="space-y-5">
                        {allowedOrders.length === 0 ? (
                            <div className="py-12 text-center opacity-30 italic text-[13px]">No active orders for this patient in {activeDepartment.name}</div>
                        ) : (
