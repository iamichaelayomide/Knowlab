...
export default function UserManagementPage() {
  ...
  return (
    <div className="kl-page">
      <div className="mb-5">
        <h1 className="text-[var(--kl-text)] text-[24px] font-semibold mb-1">HOD User Management</h1>
        <p className="text-[var(--kl-text-muted)] text-[13px]">
          HOD can create users directly and approve/reject supervisor user requests.
        </p>
      </div>

      <div className="grid lg:grid-cols-2 gap-5">
        <div className="bg-[var(--kl-surface)] border border-[var(--kl-border)] rounded-[18px] p-4 sm:p-5">
          <h2 className="text-[var(--kl-text)] font-semibold text-[16px] mb-3">Create User (HOD direct)</h2>
          <form onSubmit={directCreate} className="space-y-3">
            <input value={name} onChange={e => setName(e.target.value)} required placeholder="Full name" className="w-full border border-[var(--kl-border)] rounded-[12px] px-3 py-2 text-[13px]" />
            <input value={email} onChange={e => setEmail(e.target.value)} required type="email" placeholder="Email" className="w-full border border-[var(--kl-border)] rounded-[12px] px-3 py-2 text-[13px]" />
            <input value={unit} onChange={e => setUnit(e.target.value)} required placeholder="Unit" className="w-full border border-[var(--kl-border)] rounded-[12px] px-3 py-2 text-[13px]" />
            <select value={role} onChange={e => setRole(e.target.value as UserRole)} className="w-full border border-[var(--kl-border)] rounded-[12px] px-3 py-2 text-[13px]">
        ...
        <div className="bg-[var(--kl-surface)] border border-[var(--kl-border)] rounded-[18px] p-4 sm:p-5">
          <h2 className="text-[var(--kl-text)] font-semibold text-[16px] mb-3">Supervisor Requests Pending Approval</h2>
              ...
              <div key={req.id} className="border border-[#e3edff] rounded-[12px] p-3">
                ...
                <div className="flex gap-2 mt-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-[12px] rounded-[10px] px-2.5 py-1.5 bg-[#e8f8f1] dark:bg-[rgba(28,123,86,0.18)] text-[#1c7b56] dark:text-[#88e0ba]"
                  >
                    <CheckCircle2 size={12} /> Approve
                  </button>
                  <button
                    onClick={() => handleDecision(req.id, 'rejected')}
                    className="inline-flex items-center gap-1 text-[12px] rounded-[10px] px-2.5 py-1.5 bg-[#fde9e9] dark:bg-[rgba(177,67,67,0.18)] text-[#b14343] dark:text-[#fca5a5]"
                  >
      ...
      <div className="bg-[var(--kl-surface)] border border-[var(--kl-border)] rounded-[18px] p-4 sm:p-5 mt-5">
          ...
          {createdOrDecided.map(req => (
            <div key={req.id} className="border border-[#e3edff] rounded-[10px] p-2.5 flex items-center justify-between">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                req.decision === 'approved' ? 'bg-[#e8f8f1] dark:bg-[rgba(28,123,86,0.18)] text-[#1c7b56] dark:text-[#88e0ba]' : 'bg-[#fde9e9] dark:bg-[rgba(177,67,67,0.18)] text-[#b14343] dark:text-[#fca5a5]'
              }`}>
                {req.decision}
              </span>
            </div>
