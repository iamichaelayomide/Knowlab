...
export default function TrainingMgmtPage() {
  ...
  const getStatusColor = (status: string | undefined) => {
    if (!status) return 'bg-[var(--kl-surface-tinted)] text-[var(--kl-text-muted)]';
    if (status === 'completed') return 'bg-[#e8f8f1] dark:bg-[rgba(28,123,86,0.18)] text-[#1c7b56] dark:text-[#88e0ba]';
    if (status === 'overdue') return 'bg-[#fde9e9] dark:bg-[rgba(177,67,67,0.18)] text-[#b14343] dark:text-[#fca5a5]';
    if (status === 'in_progress') return 'bg-[#fff0db] dark:bg-[rgba(154,97,21,0.18)] text-[#9a6115] dark:text-[#f3c26f]';
    return 'bg-[var(--kl-surface-tinted)] text-[var(--kl-text-muted)]';
  };
  ...
  return (
    <div className="kl-page">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[var(--kl-text)] font-semibold text-[24px] mb-1">Training Management</h1>
          <p className="text-[var(--kl-text-muted)] text-[14px]">Team training compliance matrix - {overallCompliance}% overall</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-[var(--kl-surface)] rounded-[20px] border border-[var(--kl-border)] p-5">
          ...
          <p className="text-[var(--kl-text-muted)] text-[12px]">staff members</p>
        </div>
        <div className="bg-[#e8f8f1] dark:bg-[rgba(28,123,86,0.18)] rounded-[20px] border border-transparent p-5">
          <div className="flex items-center gap-2 mb-2">
            <CheckCircle2 size={16} className="text-[#1c7b56] dark:text-[#88e0ba]" />
            <span className="text-[#1c7b56] dark:text-[#88e0ba] text-[13px]">Compliance</span>
          </div>
          <p className="text-[#1c7b56] dark:text-[#88e0ba] font-bold text-[24px]">{overallCompliance}%</p>
          <p className="text-[#1c7b56] dark:text-[#88e0ba] text-[12px]">{completedCells} of {totalCells} completions</p>
        </div>
        <div className="bg-[#fde9e9] dark:bg-[rgba(177,67,67,0.18)] rounded-[20px] border border-transparent p-5">
          <div className="flex items-center gap-2 mb-2">
            <AlertTriangle size={16} className="text-[#b14343] dark:text-[#fca5a5]" />
            <span className="text-[#b14343] dark:text-[#fca5a5] text-[13px]">Overdue</span>
          </div>
          <p className="text-[#b14343] dark:text-[#fca5a5] font-bold text-[24px]">{overdueCells}</p>
          <p className="text-[#b14343] dark:text-[#fca5a5] text-[12px]">items need attention</p>
        </div>
      </div>
      ...
      {view === 'matrix' ? (
        ...
        <div className="bg-[var(--kl-surface)] rounded-[24px] border border-[var(--kl-border)] overflow-x-auto shadow-[var(--kl-shadow)]">
                ...
                {TRAINING_MODULES.map(mod => (
                  <th key={mod.id} className="text-center p-3 text-[var(--kl-text-muted)] font-semibold text-[10px] uppercase tracking-[0.8px] min-w-[90px]">
                    <div className="flex flex-col items-center gap-1">
                      ...
                      {mod.mandatory && <span className="text-[9px] bg-[#fde9e9] dark:bg-[rgba(177,67,67,0.18)] text-[#b14343] dark:text-[#fca5a5] px-1.5 py-0.5 rounded-full">REQ</span>}
                    </div>
                  </th>
                ))}
                <th className="text-center p-3 text-[var(--kl-text-muted)] font-semibold text-[10px] uppercase tracking-[0.8px]">Score</th>
              </tr>
            </thead>
            <tbody>
              {staffList.map(staff => {
                  ...
                  <tr key={staff.id} className="border-b border-[#f4f8ff] last:border-0 hover:bg-[var(--kl-surface-soft)] transition-colors">
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[var(--kl-text)] font-medium text-[13px]">{staff.name}</span>
                      </div>
                    </td>
                    {TRAINING_MODULES.map(mod => {
                    ...
                    <td className="text-center p-3">
                      <span className={`font-semibold text-[13px] ${
                        scorePercent === 100 ? 'text-[#1c7b56] dark:text-[#88e0ba]' : scorePercent >= 70 ? 'text-[#9a6115] dark:text-[#f3c26f]' : 'text-[#b14343] dark:text-[#fca5a5]'
                      }`}>{scorePercent}%</span>
                    </td>
                  </tr>
                );
          ...
          <div className="flex items-center gap-4 px-4 py-3 border-t border-[#f4f8ff] text-[11px] text-[var(--kl-text-muted)]">
            ...
            {[
              ...
              { icon: <AlertTriangle size={11} />, label: 'Overdue', bg: 'bg-[#fde9e9] dark:bg-[rgba(177,67,67,0.18)] text-[#b14343] dark:text-[#fca5a5]' },
              { icon: <Clock size={11} />, label: 'In Progress', bg: 'bg-[#fff0db] dark:bg-[rgba(154,97,21,0.18)] text-[#9a6115] dark:text-[#f3c26f]' },
              { icon: <span>-</span>, label: 'Not Started', bg: 'bg-[var(--kl-surface-tinted)] text-[var(--kl-text-muted)]' },
            ].map(item => (
        ...
        <div className="space-y-4">
          {TRAINING_MODULES.map(mod => {
            ...
            return (
              <div key={mod.id} className="bg-[var(--kl-surface)] rounded-[20px] border border-[var(--kl-border)] p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      <h3 className="text-[var(--kl-text)] font-semibold text-[15px]">{mod.title}</h3>
                      {mod.mandatory && <span className="bg-[#fde9e9] dark:bg-[rgba(177,67,67,0.18)] text-[#b14343] dark:text-[#fca5a5] text-[10px] font-semibold px-2 py-0.5 rounded-full">MANDATORY</span>}
                    </div>
                    <p className="text-[var(--kl-text-muted)] text-[12px]">
                      {joinWithSeparator([mod.category, mod.duration, `Pass mark: ${mod.passingScore}%`])}
                    </p>
                  </div>
                  <span className={`font-bold text-[18px] ${pct === 100 ? 'text-[#1c7b56] dark:text-[#88e0ba]' : pct >= 70 ? 'text-[#9a6115] dark:text-[#f3c26f]' : 'text-[#b14343] dark:text-[#fca5a5]'}`}>{pct}%</span>
                </div>
                <div className="w-full bg-[var(--kl-surface-tinted)] rounded-full h-1.5 mb-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {remaining > 0 && (
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium bg-[var(--kl-surface-tinted)] text-[var(--kl-text-muted)]">
                      +{remaining} more
                    </div>
                  )}
                </div>
