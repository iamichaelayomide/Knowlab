...
export default function QCLogPage() {
  ...
  const stats = {
    ...
    failed: QC_LOGS.filter(q => q.overallStatus === 'failed').length,
  ...
  return (
    <div className="p-6 max-w-[1000px]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[
          ...
          <button
            ...
            className={`rounded-[18px] p-4 text-left transition-all border-2 ${filter === s.f ? 'border-current' : 'border-transparent'}`}
      ...
      <div className="bg-white rounded-[24px] border border-[#d3def5] overflow-hidden shadow-[0px_6px_18px_0px_rgba(15,40,90,0.05)]">
        <div className="px-5 py-3 border-b border-[#f4f8ff] bg-[#f9fbff] hidden sm:grid grid-cols-[1fr_1fr_1fr_1fr_auto] gap-4">
          {['Date', 'Level', 'Shift · Analyst', 'Overall Status', 'Action'].map(h => (
        ...
        {filtered.map(entry => {
          ...
          return (
            <div key={entry.id} className="border-b border-[#f4f8ff] last:border-0">
              <div className="px-5 py-4 grid sm:grid-cols-[1fr_1fr_1fr_1fr_auto] gap-3 sm:gap-4 items-center hover:bg-[#f9fbff] transition-colors">
                {/* Date */}
                <div>
                  <p className="text-[#11203b] font-medium text-[13px]">{new Date(entry.date).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })}</p>
                  <p className="text-[#73839f] text-[11px]">{entry.analyzer.split(' ')[0]} {entry.analyzer.split(' ')[1]}</p>
                </div>
                {/* Level */}
                <div>
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-[12px] font-medium px-2.5 py-1 rounded-full flex items-center gap-1.5 ${
                  ...
                  {!isReviewed && (
                    <span className="text-[10px] bg-[#e3edff] text-[#1c5eff] font-semibold px-2 py-0.5 rounded-full">NEW</span>
                  )}
                </div>
                {/* Actions */}
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-[#475a7d] hover:text-[#1c5eff] transition-colors"
                  >
              ...
              {/* Expanded Detail */}
              {isExpanded && (
                <div className="border-t border-[#f4f8ff] bg-[#f9fbff] p-5">
                  {/* Parameter Results */}
                    ...
                    <table className="w-full text-left min-w-[500px]">
                      <thead>
                        <tr className="border-b border-[#eef4ff]">
                          {['Parameter', 'Measured', 'Target', 'SD Value', 'Deviation (SD)', 'Status'].map(h => (
                            <th key={h} className="text-[#73839f] font-semibold text-[10px] uppercase tracking-[0.8px] pb-2 pr-4">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {entry.results.map((r, i) => (
                          <tr key={i} className="border-b border-[#f4f8ff] last:border-0">
                            <td className="py-2.5 pr-4 text-[#11203b] font-medium text-[12px]">{r.parameter}</td>
                            <td className="py-2.5 pr-4 text-[#11203b] font-mono text-[12px]">{r.measuredValue}</td>
                            <td className="py-2.5 pr-4 text-[#475a7d] font-mono text-[12px]">{r.targetValue}</td>
                            <td className="py-2.5 pr-4 text-[#475a7d] font-mono text-[12px]">±{r.sdValue}</td>
                            <td className={`py-2.5 pr-4 font-mono font-semibold text-[12px] ${
                              Math.abs(r.sdDeviation) > 2 ? 'text-[#b14343]' :
                              Math.abs(r.sdDeviation) > 1.5 ? 'text-[#9a6115]' :
                              'text-[#1c7b56]'
                            }`}>{r.sdDeviation > 0 ? '+' : ''}{r.sdDeviation.toFixed(2)}</td>
                            <td className="py-2.5">
                              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                                r.status === 'pass' ? 'bg-[#e8f8f1] text-[#1c7b56]' :
                                r.status === 'warning' ? 'bg-[#fff0db] text-[#9a6115]' :
                                'bg-[#fde9e9] text-[#b14343]'
                              }`}>
