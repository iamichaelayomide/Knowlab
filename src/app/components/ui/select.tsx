...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
          "kl-input data-[placeholder]:text-[var(--text-tertiary)] [&_svg:not([class*='text-'])]:text-[var(--text-secondary)] focus-visible:border-[var(--surface-border-strong)] focus-visible:ring-[var(--accent-glow)] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive flex w-full items-center justify-between gap-2 border border-[var(--surface-border-strong)] bg-[var(--surface-card)] px-4 py-2 text-[14px] text-[var(--text-primary)] whitespace-nowrap shadow-xs transition-[color,box-shadow,border-color,background] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 data-[size=default]:h-[44px] data-[size=sm]:h-9 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    >
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      ...
      className={cn("px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-tertiary)]", className)}
      {...props}
    />
  );
}
...
function SelectSeparator({
  ...
  return (
    <SelectPrimitive.Separator
      ...
      className={cn("pointer-events-none mx-2 my-1 h-px bg-[var(--surface-border)]", className)}
      {...props}
    />
  );
}
